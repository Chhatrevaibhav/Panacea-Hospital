...
export default function Calls() {
  ...
  const { isOpen: isNewCallOpen, onOpen: onNewCallOpen, onClose: onNewCallClose } = useDisclosure();
  const { isOpen: isEditCallOpen, onOpen: onEditCallOpen, onClose: onEditCallClose } = useDisclosure();
  ...
  const handleMarkFollowUpCompleted = async (call) => {
    try {
      await callService.markFollowUpCompleted(call.id, 'Follow-up completed');
      const updatedCalls = calls.map(c => 
        c.id === call.id ? { ...c, follow_up_required: false } : c
      );
    ...
    return (
      <Box pt={{ base: "130px", md: "80px", xl: "80px" }} display="flex" justifyContent="center" alignItems="center" minH="50vh">
        <VStack spacing={4}>
          ...
          <Text color={textColor}>Loading calls...</Text>
        </VStack>
      </Box>
    );
  }
    ...
    return (
      <Box pt={{ base: "130px", md: "80px", xl: "80px" }}>
        <Alert status="error">
          <AlertIcon />
          <Box>
            <Text fontWeight="bold">Error loading calls</Text>
  ...
  return (
    <Box pt={{ base: "130px", md: "80px", xl: "80px" }}>
      <VStack spacing={6} align="stretch">
        {/* Header */}
        <Box>
          <Heading size="lg" mb={2} color={textColor}>Calls</Heading>
          <Text color="secondaryGray.600">Manage patient calls and conversation history.</Text>
        </Box>
                          ...
                          <HStack spacing={2} justify="center">
                            ...
                            {call.follow_up_required && (
                              <Button
                                ...
                                colorScheme="green"
                                variant="outline"
                                onClick={() => handleMarkFollowUpCompleted(call)}
      ...
      <LogCallModal 
        ...
        onClose={onNewCallClose}
        onCallLogged={refreshCalls}
      />
    </Box>
  );
}